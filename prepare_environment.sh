...
CURL_OUTPUT=$(curl -H "Content-Type: application/json" -X POST -d '{"refresh_token":"'$KEYCLOAK_TOKEN'"}' https://auth.${URL_PART}/api/token/refresh)
...
if [[ $(curl -X GET -H "Authorization: Bearer ${ACTIVE_TOKEN}" https://auth.${URL_PART}/api/token?for=${OSO_MASTER_URL} \
